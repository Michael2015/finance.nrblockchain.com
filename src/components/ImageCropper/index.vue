    ...
    <div class="vicp-wrap">
      ...
      <div v-show="step == 1" class="vicp-step1">
        <div class="vicp-drop-area" @dragleave="preventDefault" @dragover="preventDefault" @dragenter="preventDefault" @click="handleClick" @drop="handleChange">
          ...
          <span v-show="loading !== 1" class="vicp-hint">请选取或拖动图片上传</span>
          <span v-show="!isSupported" class="vicp-no-supported-hint">不支持该图片文件</span>
      ...
      <div v-if="step == 2" class="vicp-step2">
        <div class="vicp-crop">
          <div v-show="true" class="vicp-crop-left">
            ...
            <div style="padding: 10px 0; color: #999">调整图片尺寸和位置（建议尺寸： {{width/100}}：{{height/100}}）</div>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      sourceImgContainer: { // sic
        width: 320,
        height: 241 // 如果生成图比例与此一致会出现bug，先改成特殊的格式吧，哈哈哈
      },
      // 原图展示属性
      scale: {
  ...
  methods: {
    ...
    upload() {
      let that = this,
        ...
        fmData = new FormData()
      fmData.append(field, data2blob(createImgUrl, mime), picType + ki + '.' + imgFormat)
      // 添加其他参数
      ...
      request({
        ...
        that.loading = 2
        // uploadProgress()
        console.log('上传返回的图片>>'+ resData.data)
  ...
  created() {
    ...
    console.log(this.ki, this.picType)
  },
  destroyed() {
